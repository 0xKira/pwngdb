    ...
    def stop(self):
        ...
        if overlap and status == "error":
            if DEBUG:
                ...
                print(
                    "%-40s = 0x%x \033[31m overlap detected !! (0x%x)\033[37m"
                    % (msg, chunk["addr"] + capsize * 2, overlap["addr"]))
                print(
                    "\033[34m>--------------------------------------------------------------------------------------<\033[37m"
                )
            else:
                print("\033[31moverlap detected !! (0x%x)\033[37m" %
                      overlap["addr"])
        ...
        allocmemoryarea[hex(chunk["addr"])] = copy.deepcopy(
        ...
        if hex(chunk["addr"]) in freerecord:
            ...
            if chunk["size"] != freechunk["size"]:
                ...
                freerecord[hex(splitchunk["addr"])] = copy.deepcopy(
                    (splitchunk["addr"],
                     splitchunk["addr"] + splitchunk["size"], splitchunk))
...
#        overlap,status = check_overlap(chunk["addr"],chunk["size"],freememoryarea)
        ...
        if overlap and status == "error":
            if DEBUG:
                msg = "\033[32mfree(0x%x)\033[37m (size = 0x%x)" % (
                    result, chunk["size"])
                print(
                    "\033[34m>--------------------------------------------------------------------------------------<\033[37m"
                )
                print(
                    "%-25s \033[31m double free detected !! (0x%x(size:0x%x))\033[37m"
                    % (msg, overlap["addr"], overlap["size"]))
                print(
                    "\033[34m>--------------------------------------------------------------------------------------<\033[37m",
                    end="")
            else:
                print("\033[31mdouble free detected !! (0x%x)\033[37m" %
                      overlap["addr"])
        ...
        if chunk["size"] <= 0x80:
            freerecord[hex(chunk["addr"])] = copy.deepcopy(
                (chunk["addr"], chunk["addr"] + chunk["size"], chunk))
            if DEBUG:
                print("")
        ...
        nextchunk["size"] = int(
            gdb.execute(cmd, to_string=True).split(":")[1].strip(),
            16) & 0xfffffffffffffff8
        cmd = "x/" + word + hex(nextchunk["addr"] + nextchunk["size"] +
                                capsize)
        nextinused = int(
...
def get_fast_bin(arena=None):
    ...
    fastbinsY = int(
        gdb.execute(cmd, to_string=True).split(":")[0].split()[0].strip(), 16)
    for i in range(fastbinsize - 3):
        ...
        while chunk["addr"] and not is_overlap[0]:
            ...
            freememoryarea[hex(chunk["addr"])] = copy.deepcopy(
                (chunk["addr"], chunk["addr"] + (capsize * 2) * (i + 2),
                 chunk))
...
def get_curthread():
    ...
    thread_id = int(
        gdb.execute(cmd,
                    to_string=True).split("thread is")[1].split()[0].strip())
    return thread_id
...
def get_max_thread():
    cmd = "info thread"
    max_thread = int(
        gdb.execute(cmd, to_string=True).replace(
            "*", "").split("\n")[-2].split()[0].strip())
    return max_thread
...
def get_tcache():
    ...
    try:
        tcache_max_bin = int(
            gdb.execute("x/" + word + " &mp_.tcache_bins",
                        to_string=True).split(":")[1].strip(), 16)
        try:
            tcache_enable = True
...
def get_smallbin(arena=None):
    ...
    for size in range(capsize * 4, max_smallbin_size, capsize * 2):
        ...
        cmd = "x/" + word + hex(
            bins_addr + idx * capsize * 2)  # calc the smallbin index
        chunkhead["addr"] = int(
            gdb.execute(cmd, to_string=True).split(":")[1].strip(), 16)
        try:
...
def get_largebin(arena=None):
    ...
    for idx in range(64, 128):
        chunkhead = {}
        cmd = "x/" + word + hex(bins_addr + idx * capsize * 2 -
                                2 * capsize)  # calc the largbin index
        chunkhead["addr"] = int(
            gdb.execute(cmd, to_string=True).split(":")[1].strip(), 16)
        try:
            bins = trace_normal_bin(chunkhead, arena)
...
def unlinkable(chunkaddr, fd=None, bk=None):
    ...
    try:
        ...
        if chunk_size != next_prev_size:
            ...
            if chunkaddr != fd_bk:
                print(
                    "\033[32mUnlinkable :\033[1;31m False (FD->bk(0x%x) != (0x%x)) \033[37m "
                    % (fd_bk, chunkaddr))
            else:
                print(
                    "\033[32mUnlinkable :\033[1;31m False (BK->fd(0x%x) != (0x%x)) \033[37m "
                    % (bk_fd, chunkaddr))
    except:
        print(
            "\033[32mUnlinkable :\033[1;31m False (FD or BK is corruption) \033[37m "
        )
    ...
    try:
        ...
        if is_mmapd:
            ...
            if size <= capsize * 0x10:  #fastbin
                ...
                if chunkaddr == old:
                    print(
                        "\033[32mFreeable :\033[1;31m false -> Double free chunkaddr(0x%x) == 0x%x )\033[37m"
                        % (chunkaddr, old))
                    return
            else:
                if chunkaddr == top["addr"]:
                    print(
                        "\033[32mFreeable :\033[1;31m False -> Free top chunkaddr(0x%x) == 0x%x )\033[37m"
                        % (chunkaddr, top["addr"]))
                    return
                ...
                if nextchunk >= top["addr"] + topsize:
                    print(
                        "\033[32mFreeable :\033[1;31m False -> Out of top chunkaddr(0x%x) > 0x%x )\033[37m"
...
def get_heapbase():
    if (main_arena and not enable_thread) or thread_arena == main_arena:
        heapbase = int(
            gdb.execute("x/" + word + " &mp_.sbrk_base",
                        to_string=True).split(":")[1].strip(), 16)
    elif thread_arena:
        arena_size = int(
            gdb.execute("p sizeof(main_arena)",
                        to_string=True).split("=")[1].strip(), 16)
...
def chunkinfo(victim):
    ...
    try:
        ...
        bk = int(gdb.execute(cmd, to_string=True).split(":")[1].strip(), 16)
        cmd = "x/" + word + hex(chunkaddr +
                                (size & 0xfffffffffffffff8) + capsize)
        nextsize = int(
            gdb.execute(cmd, to_string=True).split(":")[1].strip(), 16)
        status = nextsize & 1
        print("==================================")
        ...
        print("\033[32msize :\033[37m 0x%x                  " %
              (size & 0xfffffffffffffff8))
        print("\033[32mprev_inused :\033[37m %x                    " %
              (size & 1))
        print("\033[32mis_mmap :\033[37m %x                    " % (size & 2))
        print("\033[32mnon_mainarea :\033[37m %x                     " %
              (size & 4))
        if not status:
        ...
        if status:
            if chunkaddr in fastchunk:
                ...
                if (size & 0xfffffffffffffff8) <= 0x80:
                    ...
                    cmd = "x/" + word + hex(
                        next_chunk + (nextsize & 0xfffffffffffffff8) + capsize)
                    ...
                    if not prev_status:  #if prev chunk is freed
                        ...
                        if next_chunk == top["addr"]:  #if next chunk is top
                            ...
                            print(
                                "The chunk will merge into \033[1;33m0x%x\033[37m"
                                % prev_chunk)
                            print(
                                "\033[32mUnlink info : \033[1;33m0x%x\033[37m"
                                % prev_chunk)
...
def putfastbin(arena=None):
    ...
    for i, bins in enumerate(fastbin):
        ...
        print(
            "\033[32m(0x%02x)     fastbin[%d]:\033[37m " % (cursize, i),
            end="")
        for chunk in bins:
            if "memerror" in chunk:
                print(
                    "\033[31m0x%x (%s)\033[37m" % (chunk["addr"],
                                                   chunk["memerror"]),
                    end="")
            elif chunk["size"] != cursize and chunk["addr"] != 0:
                print(
                    "\033[36m0x%x (size error (0x%x))\033[37m" %
                    (chunk["addr"], chunk["size"]),
                    end="")
            elif chunk["overlap"] and chunk["overlap"][0]:
                print(
                    "\033[31m0x%x (overlap chunk with \033[36m0x%x(%s)\033[31m )\033[37m"
                    % (chunk["addr"], chunk["overlap"][0]["addr"],
                       chunk["overlap"][1]),
                    end="")
            elif chunk == bins[0]:
...
def put_tcache():
    ...
    for i, entry in enumerate(tcache_entry):
        ...
        if len(tcache_entry[i]) > 0:
            print(
                "\033[33;1m(0x%02x)   tcache_entry[%d]:\033[37m " % (cursize,
                                                                     i),
                end="")
        for chunk in entry:
            if "memerror" in chunk:
                print(
                    "\033[31m0x%x (%s)\033[37m" % (chunk["addr"] + capsize * 2,
                                                   chunk["memerror"]),
                    end="")
            elif chunk["overlap"] and chunk["overlap"][0]:
                print(
                    "\033[31m0x%x (overlap chunk with \033[36m0x%x(%s)\033[31m )\033[37m"
                    % (chunk["addr"] + capsize * 2,
                       chunk["overlap"][0]["addr"], chunk["overlap"][1]),
                    end="")
            elif chunk == entry[0]:
                print(
                    "\033[34m0x%x\033[37m" % (chunk["addr"] + capsize * 2),
                    end="")
            else:
                if print_overlap:
...
def putheapinfo(arena=None):
    ...
    if unsortbin and len(unsortbin) > 0:
        ...
        for chunk in unsortbin:
            if "memerror" in chunk:
                ...
                print(
                    "\033[34m0x%x\033[37m \33[33m(size : 0x%x)\033[37m" %
                    (chunk["addr"], chunk["size"]),
                    end="")
            else:
                print(
                    "0x%x \33[33m(size : 0x%x)\033[37m" % (chunk["addr"],
                                                           chunk["size"]),
                    end="")
    ...
    for size, bins in smallbin.items():
        ...
        print(
            "\033[33m(0x%03x)  %s[%2d]:\033[37m " % (int(size, 16), "smallbin",
                                                     idx),
            end="")
        for chunk in bins:
            if "memerror" in chunk:
    ...
    for idx, bins in largebin.items():
        ...
        for chunk in bins:
            if "memerror" in chunk:
                ...
                print(
                    "\033[31m0x%x (incorrect bin size :\033[36m 0x%x\033[31m)\033[37m"
                    % (chunk["addr"], chunk["size"]),
                    end="")
            elif chunk == bins[-1]:
                print(
...
def putheapinfoall():
    ...
    for thread_id in range(1, max_thread + 1):
        if thread_id == cur_thread_id:
            print(
                "\033[33;1m" + ("  Thread " + str(thread_id) + "  ").center(
                    50, "=") + "\033[0m",
                end="")
        else:
            print(
                ("  Thread " + str(thread_id) + "  ").center(50, "="), end="")
...
def parse_heap(arena=None):
    ...
    if not chunkaddr:
        ...
        return
    print('\033[1;33m{:<20}{:<20}{:<21}{:<20}{:<18}{:<18}\033[0m'.format(
    ...
    while chunkaddr != top["addr"]:
        try:
            ...
            if size == 0:
                print(
                    "\033[31mCorrupt ?! \033[0m(size == 0) (0x%x)" % chunkaddr)
                break
            if status:
                if chunkaddr in fastchunk:
                ...
                msg = "\033[1;34m Freed \033[0m"
                print('0x{:<18x}0x{:<18x}0x{:<18x}{:<16}{:>18}{:>18}'.format(
...
def fake_fast(addr, size):
    ...
    for offset in range(chunk_size - 4):
        ...
        if fastbin_idx(fake_size) == idx:
            if ((fake_size & 2 == 2) and
                (fake_size & 4 == 4)) or (fake_size & 4 == 0):
