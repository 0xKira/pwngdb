...
import struct
import os
from .utils import get_arch, get_libc_version, reveal_ptr
...
class Free_Bp_handler(gdb.Breakpoint):
    def stop(self):
        ...
        cmd = "x/" + word + hex(next_chunk["addr"] + next_chunk["size"] + capsize)
    ...
    if data:
        for key, (start, end, chunk) in data.items():
            if (addr >= start and addr < end) or ((addr + size) > start and
                                                  (addr + size) < end) or ((addr < start) and ((addr + size) >= end)):
                return chunk, "error"
    else:
...
def get_all_threads():
    cmd = "info threads"
    all_threads = [
        int(line.split()[0].strip()) for line in gdb.execute(cmd, to_string=True).replace("*", "").split("\n")[1:-1]
    ]
    return all_threads
...
def get_tcache():
    ...
    try:
        ...
        try:
            ...
            tps_size = int(gdb.execute("p/x sizeof(*tcache)", to_string=True).split("=")[1].strip(), 16)
            if capsize == 4:
                if tps_size > 0x140:
...
def get_smallbin(arena=None):
    global smallbin, tcache_enable, bin_corrupt
    if not arena:
        arena = main_arena
    smallbin = {}
    ...
    cur_size = 8 if tcache_enable else capsize
    for size in range(capsize * 4, max_smallbin_size, cur_size * 2):
        chunkhead = {}
        idx = int((size / (capsize * 2))) - 1
        if tcache_enable and capsize == 4:
            idx = int(size / (cur_size * 2))
        else:
            idx = int(size / (cur_size * 2)) - 1
    ...
    try:
        ...
        if is_mmapd:
            ...
            if chunkaddr > (2**(capsize * 8) - (size & 0xfffffffffffffff8)):
                print("\033[32mFreeable :\033[1;31m False -> Invalid pointer chunkaddr (0x%x) > -size (0x%x)\033[37m" %
...
def putfastbin(arena=None):
    ...
    for i, bins in enumerate(fastbin):
        cur_size = (capsize * 2) * (i + 2)
        print("\033[32m(0x%02x)     fastbin[%d]:\033[37m " % (cur_size, i), end="")
        for chunk in bins:
            if "memerror" in chunk:
                print("\033[31m0x%x (%s)\033[37m" % (chunk["addr"], chunk["memerror"]), end="")
            elif chunk["size"] != cur_size and chunk["addr"] != 0:
                ...
                print("\033[31m0x%x (overlap chunk with \033[36m0x%x(%s)\033[31m )\033[37m" %
                      (chunk["addr"], chunk["overlap"][0]["addr"], chunk["overlap"][1]),
                      end="")
            elif chunk == bins[0]:
...
def put_tcache():
    ...
    for i, entry in enumerate(tcache_entry):
        ...
        if len(tcache_entry[i]) > 0:
            print("\033[33;1m(0x%02x)   tcache_entry[%d]\033[32m(%d)\033[33;1m:\033[37m " %
                  (cur_size, i, tcache_count[i]),
                  end="")
        ...
        for chunk in entry:
            if "memerror" in chunk:
                ...
                print("\033[31m0x%x (overlap chunk with \033[36m0x%x(%s)\033[31m )\033[37m" %
                      (chunk["addr"] + capsize * 2, chunk["overlap"][0]["addr"], chunk["overlap"][1]),
                      end="")
            elif chunk == entry[0]:
    ...
    for idx, bins in largebin.items():
        ...
        for chunk in bins:
            if "memerror" in chunk:
                ...
                print("\033[31m0x%x (incorrect bin size :\033[36m 0x%x\033[31m)\033[37m" %
        ...
        try:
            ...
            if status:
                if chunkaddr in fastchunk or chunkaddr in all_tcache_entry:
                    ...
                    print('0x{:<18x}0x{:<18x}0x{:<18x}{:<16}{:>18}{:>18}'.format(chunkaddr, prev_size, size, msg,
                                                                                 hex(fd), "None"))
                else:
